...
func TestServiceOptions(t *testing.T) {
	...
	//os.Setenv("KN_OPENSHIFT_CI", "1")
	var uid int64 = 1000
	if _, ok := os.LookupEnv("KN_OPENSHIFT_CI"); ok {
		// kubectl get ns kne2etests0 -o="jsonpath={.metadata.annotations.openshift\.io/sa\.scc\.uid-range}"
		jsonpath := "jsonpath={.metadata.annotations.openshift\\.io/sa\\.scc\\.uid-range}"
		out, err := RunKubectl("", "get", "ns", test.kn.namespace, "-o", jsonpath)
		assert.NilError(t, err)
		uid, err = strconv.ParseInt(strings.Split(out, "/")[0], 10, 64)
		assert.NilError(t, err)
	}
