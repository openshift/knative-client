...
func TestSourceKafka(t *testing.T) {
	...
	t.Log("test kn-source-kafka --help")
	e2eTest.knSourceKafkaHelp(t, r)
...
func (et *e2eTest) knSourceKafkaHelp(t *testing.T, r *test.KnRunResultCollector) {
	out := r.KnTest().Kn().RunNoNamespace("source", "kafka", "--help")
	r.AssertNoError(out)
	assert.Check(t, util.ContainsAllIgnoreCase(out.Stdout, "kafka", "eventing", "sources"))
}
